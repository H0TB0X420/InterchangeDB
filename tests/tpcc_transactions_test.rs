...
fn seed_baseline(s: &mut Session<BTreeEngine>) {
    ...
    s.execute("INSERT INTO district VALUES (10, 1, 0, 100)")
        .unwrap();
    // Two customers, different last names.
    s.execute("INSERT INTO customer VALUES (1, 5000, 'kim', 'alice')")
        .unwrap();
    s.execute("INSERT INTO customer VALUES (2, 3000, 'kim', 'bob')")
        .unwrap();
    s.execute("INSERT INTO customer VALUES (3, 1000, 'smith', 'carol')")
        .unwrap();
    // Three items.
...
fn new_order_runs_through_lookups_inserts_and_increment_update() {
    ...
    let ps_next = s
        .prepare("SELECT d_next_o_id FROM district WHERE d_id = $1")
        .unwrap();
    ...
    let ps_order = s.prepare("INSERT INTO orders VALUES ($1, $2, 0)").unwrap();
    ...
    let r = rows(
        s.execute("SELECT o_id FROM orders ORDER BY o_id ASC")
            .unwrap(),
    );
    assert_eq!(r.len(), 2);
    assert_eq!(r[0][0], Value::Int32(100));
...
fn payment_runs_with_c_last_fallback_via_order_by() {
    ...
    let ps_cust_by_last = s
        .prepare(
            "SELECT c_id, c_balance FROM customer WHERE c_last = $1 ORDER BY c_first ASC LIMIT 1",
        )
        .unwrap();
    let r = rows(
    ...
    let ps_h = s
        ...
        .unwrap();
    s.execute_prepared(&ps_h, &[Value::Int32(1), Value::Int32(1), amount.clone()])
        .unwrap();
}
...
fn order_status_finds_most_recent_order_via_max() {
    ...
    s.execute("INSERT INTO order_line VALUES (1, 30, 101, 100)")
        .unwrap();
    s.execute("INSERT INTO order_line VALUES (2, 30, 102, 200)")
        .unwrap();

    let ps_max = s
        .prepare("SELECT MAX(o_id) FROM orders WHERE o_c_id = $1")
        .unwrap();
...
fn delivery_drains_oldest_new_order_and_sums_lines() {
    ...
    s.execute("INSERT INTO orders VALUES (10, 1, 0)").unwrap();
    s.execute("INSERT INTO order_line VALUES (100, 10, 101, 250)")
        .unwrap();
    s.execute("INSERT INTO order_line VALUES (101, 10, 102, 750)")
        .unwrap();
    ...
    let ps_del = s
        .prepare("DELETE FROM new_orders WHERE no_o_id = $1")
        .unwrap();
    s.execute_prepared(&ps_del, &[Value::Int32(10)]).unwrap();
    let remaining = rows(
        s.execute("SELECT no_o_id FROM new_orders ORDER BY no_o_id ASC")
            .unwrap(),
    );
    ...
    let ps_cust = s
        .prepare("SELECT o_c_id FROM orders WHERE o_id = $1")
        .unwrap();
    let r = rows(s.execute_prepared(&ps_cust, &[Value::Int32(10)]).unwrap());
    let customer_id = match r[0][0] {
        Value::Int32(i) => i,
        _ => panic!(),
    };
...
fn stock_level_counts_distinct_items_via_join() {
    ...
    s.execute("INSERT INTO order_line VALUES (1, 1000, 101, 0)")
        .unwrap();
    s.execute("INSERT INTO order_line VALUES (2, 1000, 102, 0)")
        .unwrap();
    s.execute("INSERT INTO order_line VALUES (3, 1001, 101, 0)")
        .unwrap(); // duplicate i_id
    s.execute("INSERT INTO order_line VALUES (4, 1001, 103, 0)")
        .unwrap();

    // Lower stock of item 101 below threshold 30; others above.
    s.execute("UPDATE stock SET s_quantity = 5 WHERE s_i_id = 101")
        .unwrap();
    s.execute("UPDATE stock SET s_quantity = 50 WHERE s_i_id = 102")
        .unwrap();
    s.execute("UPDATE stock SET s_quantity = 100 WHERE s_i_id = 103")
        .unwrap();
